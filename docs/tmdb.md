...
## Overview
We use the [TheMovieDB API](https://developer.themoviedb.org/reference/intro/getting-started) to fetch TV and movie data for trivia games.

- **API Key**: Stored in `.env.local` as `TMBD_READ_TOKEN`.
- **Proxying**: We never call TMDB directly from the client. Instead, requests go through `/api/tmdb` in the Next.js app.
- **Helpers**: `src/lib/tmdb.ts` provides convenience functions for client components.

## Example Usage
```ts
import { tmdb } from '@/lib/tmdb';

const results = await tmdb('/search/tv', { query: 'Real Housewives' });
...
.. code:: python
    ...
    async def main():
        ...
        movie = await tmdb.movie(movie_id).details(append_to_response="credits,external_ids,images,videos") print(movie.title, movie.year)
        print(movie.tagline)
