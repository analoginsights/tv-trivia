...
async function fixedChunkedETL() {
  console.log('🚀 Starting Fixed Chunked Bravo ETL...\n')
  ...
  try {
    ...
    // Step 4: CHUNKED Upsert to Supabase (CRITICAL: Handle large datasets!)
    console.log('\n💾 Upserting data to Supabase in chunks...')
    ...
    console.error('❌ Fixed Chunked ETL failed:', error)
    throw error
  }
}
...
if (require.main === module) {
  fixedChunkedETL()
    .then(stats => {
      console.log('\n🎯 ETL Success! Data integrity maintained with chunked processing.')
