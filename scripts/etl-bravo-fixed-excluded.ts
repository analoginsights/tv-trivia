...
async function fixedExcludedETL() {
  console.log(`🚀 Starting Fixed ETL (Excluding Show ${EXCLUDED_SHOW_ID})...\n`)
  ...
  try {
    ...
    for (let page = 1; page <= ETL_PAGES; page++) {
      ...
      shows.forEach((show, idx) => {
        // Skip the problematic show
        if (show.id === EXCLUDED_SHOW_ID) {
          console.log(`  ⏩ Skipping excluded show: ${show.name} (ID: ${show.id})`)
          return
        }
    ...
    console.log(`✅ Found ${allShows.size} Bravo shows (excluding problematic show)\n`)
    ...
    for (const [showId, show] of allShows) {
      showCount++
      console.log(`\n[${showCount}/${allShows.size}] ${show.name} (ID: ${showId})`)
    ...
    // Check show distribution before upsert
    ...
    console.log('\n📊 Show distribution before upsert:')
    const sortedDist = Array.from(showDistribution.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10)
      
    for (const [showId, count] of sortedDist) {
    ...
    const finalShowCounts = new Map()
    finalAppearances?.forEach(app => {
      finalShowCounts.set(app.show_id, (finalShowCounts.get(app.show_id) || 0) + 1)
    })
    
    console.log(`\n📊 Final Show Distribution (Top 10):`)
    const finalSortedShows = Array.from(finalShowCounts.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10)
      
    for (const [showId, count] of finalSortedShows) {
      const show = showsData.find(s => s.id === showId)
      console.log(`  ${show?.name || `Show ${showId}`}: ${count} appearances`)
    }
    
    console.log(`\n🎉 Fixed Excluded ETL Complete!`)
    console.log(`\n📊 Final Statistics:`)
    ...
    console.log(`  Shows with appearances: ${finalShowCounts.size}`)
    console.log(`  Excluded show: ${EXCLUDED_SHOW_ID}`)
    ...
    return {
      ...
      guestOnly,
      both,
      showsWithAppearances: finalShowCounts.size,
      excludedShowId: EXCLUDED_SHOW_ID
    }
    
  } catch (error) {
    console.error('❌ Fixed Excluded ETL failed:', error)
    throw error
  }
}
...
if (require.main === module) {
  fixedExcludedETL()
    .then(stats => {
      console.log('\n🎯 ETL Success! Data properly distributed without problematic show.')
